...
var (
	...
	errParse       = jsonrpc1.NewError(-32700, "Parse error")
	errRequest     = jsonrpc1.NewError(-32600, "Invalid request")
	errMethod      = jsonrpc1.NewError(-32601, "Method not found")
	errParams      = jsonrpc1.NewError(-32602, "Invalid params")
	errInternal    = jsonrpc1.NewError(-32603, "Internal error")
	errServer      = jsonrpc1.NewError(-32000, "Server error")
	errServerError = jsonrpc1.NewError(-32001, "jsonrpc1.Error: json.Marshal failed")
)
...
func (*Svc) Err3(struct{}, *struct{}) error {
	return &jsonrpc1.Error{42, "some issue", map[string]int{"one": 1, "two": 2}}
}
...
func testClient(t *testing.T, cli, srv net.Conn, client *jsonrpc1.Client, method string, in interface{}, want string, wanterr *jsonrpc1.Error) {
	type Res struct {
		got string
		err error
	}
	...
	if err == nil && wanterr != nil || err != nil && (wanterr == nil || !reflect.DeepEqual(jsonrpc1.ServerError(err), wanterr)) {
...
func testClientNotify(t *testing.T, cli, srv net.Conn, client *jsonrpc1.Client, method string, in interface{}, want string, wanterr *jsonrpc1.Error) {
	type Res struct {
		got string
		err error
	}
...
func TestServerJSON(t *testing.T) {
	cases := []struct {
		...
		{`{  "id":0,"method":"Svc.Sum","params":[3,5]}`, jres8},
		// Method type
		...
		{`{"id":0,"method":"Svc.Sum"}`, jres0},
		{`{"id":0,"method":"Svc.Sum","params":[]}`, jres0},
		{
		...
		{`{"id":0,"method":"Svc.Sum","PARAMS":[]}`, jerrRequest},
		// extra key
		{`{"method":"Svc.Sum","extra":[]}`, jerrRequest},
		{`{"extra":0,"method":"Svc.Sum","params":[]}`, jerrRequest},
		{`{"id":0,"method":"Svc.Sum","extra":[]}`, jerrRequest},
		{`{"id":0,"method":"Svc.Sum","params":[],"extra":[]}`, jerrRequest},
		// default Error.Code
		{
		...
		{`{"id":0,"method":"Svc.Sum"}`, jres0},
		{`{"id":0,"method":"Svc.Sum","params":[]}`, jres0},
		{`{"id":0,"method":"Svc.Sum","params":[8]}`, jres8},
		{`{"id":0,"method":"Svc.Sum","params":[3,5]}`, jres8},
		{`{"id":0,"method":"Svc.Sum","params":[3,5,7]}`, jres8},
		{
			`{"id":0,"method":"Svc.Sum","params":{}}`,
			fmt.Sprintf(jerrParamsFmt, "json: cannot unmarshal object into Go value of type [2]int"),
		},
		...
		// Params to Slice
		{`{"id":0,"method":"Svc.SumAll"}`, jres0},
		{`{"id":0,"method":"Svc.SumAll","params":[]}`, jres0},
		{`{"id":0,"method":"Svc.SumAll","params":[8]}`, jres8},
		{`{"id":0,"method":"Svc.SumAll","params":[3,5]}`, jres8},
		{`{"id":0,"method":"Svc.SumAll","params":[3,5,7]}`, jres15},
		{
		...
		{`{"id":0,"method":"Svc.MapLen","params":{}}`, jres0},
		{`{"id":0,"method":"Svc.MapLen","params":{"a":3,"b":5}}`, jres2},
		// Params to Struct
		{
...
func TestClientResponse(t *testing.T) {
	var errBadResponseFmt = jsonrpc1.NewError(-32603, "bad response: %s")
	cases := []*struct {
		in      string
		want    float64
		wanterr *jsonrpc1.Error
	}{
		// smoke
		{jres0, 0.0, nil},
		// bad JSON
		{`x`, 0.0, jsonrpc1.NewError(-32603, "invalid character 'x' looking for beginning of value")},
		{`{jsonrpc:"2.0"}`, 0.0, jsonrpc1.NewError(-32603, "invalid character 'j' looking for beginning of object key string")},
		// bad Response type
		...
		{`{"id":0,"result":0,"error":{"code":0,"message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"result":null}`, 0.0, nil},
		{
			`{"id":0,"result":true}`,
			0.0, jsonrpc1.NewError(errInternal.Code, "json: cannot unmarshal bool into Go value of type float64"),
		},
		{
			`{"id":0,"result":false}`,
			0.0, jsonrpc1.NewError(errInternal.Code, "json: cannot unmarshal bool into Go value of type float64"),
		},
		{`{"id":0,"result":0}`, 0.0, nil},
		{
			`{"id":0,"result":"0"}`,
			0.0, jsonrpc1.NewError(errInternal.Code, "json: cannot unmarshal string into Go value of type float64"),
		},
		{
			`{"id":0,"result":[0]}`,
			0.0, jsonrpc1.NewError(errInternal.Code, "json: cannot unmarshal array into Go value of type float64"),
		},
		{
			`{"id":0,"result":{}}`,
			0.0, jsonrpc1.NewError(errInternal.Code, "json: cannot unmarshal object into Go value of type float64"),
		},
		// Error type
		{`{"id":0,"error":null}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":true}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":false}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":0}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":"0"}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":[]}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":null, "message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":true, "message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":false,"message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":-1,    "message":""}}`, 0.0, jsonrpc1.NewError(-1, "")},
		{`{"id":0,"error":{"code":"0",  "message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":[0],  "message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":{},   "message":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":null}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":true}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":false}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":0}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":""}}`, 0.0, jsonrpc1.NewError(0, "")},
		{`{"id":0,"error":{"code":0,"message":[]}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":{}}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"message":"","data":null}}`, 0.0, jsonrpc1.NewError(0, "")},
		{`{"id":0,"error":{"code":0,"message":"","data":"str"}}`, 0.0, &jsonrpc1.Error{0, "", "str"}},
		{`{"id":0,"error":{"code":0,"message":"","data":["str"]}}`, 0.0, &jsonrpc1.Error{0, "", []interface{}{"str"}}},
		// bad case
		{`{"id":0,"error":{"code":0,"message":"","data":""}}`, 0.0, errBadResponseFmt},
		{`{"ID":0,"error":{"code":0,"message":"","data":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"ERROR":{"code":0,"message":"","data":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"CODE":0,"message":"","data":""}}`, 0.0, errBadResponseFmt},
		{`{"id":0,"error":{"code":0,"MESSAGE":"","data":""}}`, 0.0, errBadResponseFmt},
	...
	for _, c := range cases {
		if c.wanterr == errBadResponseFmt {
			c.wanterr = jsonrpc1.NewError(errBadResponseFmt.Code, fmt.Sprintf(errBadResponseFmt.Message, c.in))
		}
	}
	for _, c := range cases {
		cli, srv := net.Pipe()
		...
		if err == nil && c.wanterr != nil || err != nil && (c.wanterr == nil || !reflect.DeepEqual(jsonrpc1.ServerError(err), c.wanterr)) {
...
func TestClientRequest(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"", new(*int),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to ptr"),
		},
		{
			"", true,
			``, jsonrpc1.NewError(-32603, "unsupported param type: bool"),
		},
		{
			"", new(bool),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to bool"),
		},
		{
			"", 0,
			``, jsonrpc1.NewError(-32603, "unsupported param type: int"),
		},
		{
			"", new(int),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to int"),
		},
		{
			"", [0]int{},
		...
		{
			"", make(chan int),
			``, jsonrpc1.NewError(-32603, "unsupported param type: chan"),
		},
		{
			"", new(chan int),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to chan"),
		},
		{
			"", func() {},
			``, jsonrpc1.NewError(-32603, "unsupported param type: func"),
		},
		{
			"", new(func()),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to func"),
		},
		// Looks like Go tip @ 2016-07-20 support map[int]int!
		// {
		// 	"", map[int]int{1: 2},
		// 	``, jsonrpc1.NewError(-32603, "json: unsupported type: map[int]int"),
		// },
		{
			"", map[string]int{"1": 2},
			`{"method":"","params":{"1":2},"id":0}`, nil,
		...
		{
			"", "str",
			``, jsonrpc1.NewError(-32603, "unsupported param type: string"),
		},
		{
			"", new(string),
			``, jsonrpc1.NewError(-32603, "unsupported param type: Ptr to string"),
		},
		{
			"", struct {
				A int
...
func TestCall(t *testing.T) {
	cases := []struct {
		...
		{"Svc.Err", struct{}{}, struct{}{}, jsonrpc1.NewError(-32000, "some issue")},
		{"Svc.Err", []struct{}{}, struct{}{}, jsonrpc1.NewError(-32602, "json: cannot unmarshal array into Go value of type struct {}")},

		{"Svc.Err2", struct{}{}, struct{}{}, jsonrpc1.NewError(42, "some issue")},
	}
	...
	for _, c := range cases {
		...
		if err == nil && c.wanterr != nil || err != nil && (c.wanterr == nil || *jsonrpc1.ServerError(err) != *c.wanterr) {
...
func TestClientMassError_json(t *testing.T) {
	...
	wanterr1 := jsonrpc1.NewError(-32603, "json: cannot unmarshal number into Go value of type string")
	wanterr2 := jsonrpc1.NewError(-32603, "some other Call failed to unmarshal Reply")

	call2 := client.Go("Svc.Msg", []string{"test"}, nil, nil)
	var badreply string
	err1 := client.Call("Svc.Sum", [2]int{}, &badreply)
	if err1 == nil || !reflect.DeepEqual(jsonrpc1.ServerError(err1), wanterr1) {
		t.Errorf("%serr1 = %v, wanterr1 = %v", caller(), err1, wanterr1)
	}
	<-call2.Done
	err2 := call2.Error
	if err2 == nil || !reflect.DeepEqual(jsonrpc1.ServerError(err2), wanterr2) {
